...
void dump_verilog_cmos_mux_one_basis_module(FILE* fp, 
  ...
  /* Port list */
  fprintf(fp, "input in[0:%d],\n", num_input_basis_subckt);
  fprintf(fp, "output out,\n");
  fprintf(fp, "input mem[0:%d], mem_inv[0:%d]);\n",
          num_mem - 1, num_mem - 1);
...
void dump_verilog_rram_mux_one_basis_module(FILE* fp, 
  ...
  fprintf(fp, "input in[0:%d],\n", num_input_basis_subckt);
  fprintf(fp, "output out,\n");
  fprintf(fp, "input bl[0:%d], wl[0:%d]);\n",
          num_mem - 1, num_mem - 1);
...
void dump_verilog_mux_basis_module(FILE* fp, 
  ...
  case SPICE_MODEL_STRUCTURE_MULTILEVEL:
    num_input_special_basis_subckt = spice_mux_model->size % num_input_basis_subckt;
    assert(!(0 > num_input_special_basis_subckt));
...
void dump_verilog_cmos_mux_submodule(FILE* fp,
  ...
  /* Print input ports*/
  fprintf(fp, "input %s[0:%d], \n", input_port[0]->prefix, mux_size - 1);
  /* Print output ports*/
  fprintf(fp, "output %s, \n", output_port[0]->prefix);
  switch (spice_model.structure) {
  case SPICE_MODEL_STRUCTURE_TREE:
    /* Print sram ports*/
    fprintf(fp, "input %s[0:%d], %s_inv[0:%d],\n", 
            sram_port[0]->prefix, spice_mux_arch.num_level - 1,
            sram_port[0]->prefix, spice_mux_arch.num_level - 1);
    break;
  case SPICE_MODEL_STRUCTURE_ONELEVEL:
    /* Print sram ports*/
    fprintf(fp, "input %s[0:%d], %s_inv[0:%d],\n", 
            sram_port[0]->prefix, mux_size - 1,
            sram_port[0]->prefix, mux_size - 1);
    break;
  case SPICE_MODEL_STRUCTURE_MULTILEVEL:
    /* Print sram ports*/
    fprintf(fp, "input %s[0:%d], %s_inv[0:%d],\n", 
            sram_port[0]->prefix, spice_mux_arch.num_level*spice_mux_arch.num_input_basis - 1,
            sram_port[0]->prefix, spice_mux_arch.num_level*spice_mux_arch.num_input_basis - 1);
    break;
  default:
    vpr_printf(TIO_MESSAGE_ERROR,"(File:%s,[LINE%d])Invalid structure for spice model (%s)!\n",
  ...
  if (1 == spice_model.output_buffer->exist) {
    ...
    /* There is no buffer, I create a zero resisitance between*/
    /* Resistance R<given_name> <input> <output> 0*/
    fprintf(fp, "assign mux2_l0_in0 = %s;\n",output_port[0]->prefix);
  }
