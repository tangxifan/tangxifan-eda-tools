...
void fprint_spice_mux_testbench_cb_interc(FILE* fp, 
  ...
  if (1 == src_rr_node->fan_in) {
    ...
    /* Print a MUX */
    fprint_spice_mux_testbench_cb_one_mux(fp, cur_cb_info, src_rr_node, LL_rr_node_indices);
  } 
...
int fprint_spice_mux_testbench_call_one_grid_sb_muxes(FILE* fp, 
  ...
  for (side = 0; side < cur_sb_info.num_sides; side++) {
    ...
    for (itrack = 0; itrack < cur_sb_info.chan_width[side]; itrack++) {
      ...
      case OUT_PORT:
        fprint_spice_mux_testbench_sb_one_mux(fp, cur_sb_info.x, cur_sb_info.y, side, cur_sb_info.chan_rr_node[side][itrack]);
        used++;
        break;
      case IN_PORT:
        break;
  ...
  for (ipin = 0; ipin < type->num_pins; ipin++) {
    ...
    if ((1 == type->pinloc[height][side][ipin])&&(pin_class_type == type->class_inf[class_id].type)) {
      ...
      case RECEIVER:
        ...
        fprint_spice_mux_testbench_one_cb_mux_loads(fp, &rr_node[inode], outport_name, LL_rr_node_indices);
        my_free(outport_name);
        break;
      default:
        vpr_printf(TIO_MESSAGE_ERROR, "(File:%s, [LINE%d])Invalid pin_type!\n", __FILE__, __LINE__);
 ...
 */
static 
int fprint_spice_mux_testbench_call_one_cb_tb(FILE* fp,
                                              t_rr_type chan_type,
                                              int x, int y,
                                              t_ivec*** LL_rr_node_indices) {
 ...
 */
static 
int fprint_spice_mux_testbench_call_one_sb_tb(FILE* fp, 
                                              int x, int y, 
                                              t_ivec*** LL_rr_node_indices) {
  int itrack, inode, side, ix, iy;
   ...
   * For each output of switch block, we generate a input voltage pulse
   */
    ...
    case 0:
      ...
      for (itrack = 0; itrack < chan_width[side]; itrack++) {
        ...
        case INC_DIRECTION: /* Output */
          ...
          rr_node_outport_name = fprint_spice_mux_testbench_rr_node_load_version(fp, 0, rr_node[inode], outport_name); 
          /* Free */
...
int fprint_spice_one_mux_testbench(char* formatted_spice_dir,
  ...
  case SPICE_SB_MUX_TB:
    ...
    break;
  case SPICE_CB_TB:
    mux_tb_name = "Connection Box";
    break;
  case SPICE_SB_TB:
    mux_tb_name = "Switch Block";
    break;
  default:
    vpr_printf(TIO_MESSAGE_ERROR, "(File:%s, [LINE%d]) Invalid mux_tb_type!\n", __FILE__, __LINE__);
    exit(1);
  ...
  */
  ...
  case SPICE_CB_TB:
    ...
    case CHANX:
    case CHANY:
      used = fprint_spice_mux_testbench_call_one_cb_tb(fp, cb_type, grid_x, grid_y, LL_rr_node_indices);
      break;
    default:
      vpr_printf(TIO_MESSAGE_ERROR, "(File:%s, [LINE%d]) Invalid connection_box_type!\n", __FILE__, __LINE__);
      exit(1);
  ...
  case SPICE_SB_TB:
    ...
    fprintf(fp, ".include \'%s\'\n", temp_include_file_path);
    my_free(temp_include_file_path);
    used = fprint_spice_mux_testbench_call_one_sb_tb(fp, grid_x, grid_y, LL_rr_node_indices);
    break;
  default:
    vpr_printf(TIO_MESSAGE_ERROR, "(File:%s, [LINE%d]) Invalid mux_tb_type!\n", __FILE__, __LINE__);
    exit(1);
  ...
  if (0 < testbench_mux_cnt) {
    ...
    used = 1;
  } else if ((SPICE_CB_TB == mux_tb_type)||(SPICE_SB_TB == mux_tb_type)) { 
    /* Patch the CB and SB TB*/
    if (NULL == tb_head) {
      tb_head = create_llist(1);
      tb_head->dptr = my_malloc(sizeof(t_spicetb_info));
      ((t_spicetb_info*)(tb_head->dptr))->tb_name = my_strdup(mux_testbench_file_path);
  ...
  case SPICE_CB_TB:
    ...
    for (iy = 0; iy < (ny+1); iy++) {
      for (ix = 1; ix < (nx+1); ix++) {
        ...
        used = fprint_spice_one_mux_testbench(formatted_spice_dir, circuit_name, mux_testbench_name, 
                                              include_dir_path, subckt_dir_path, LL_rr_node_indices,
                                              num_clocks, arch, ix, iy, CHANX, SPICE_CB_TB, 
                                              leakage_only);
        if (1 == used) {
          cnt += used;
        }
    ...
    for (ix = 0; ix < (nx+1); ix++) {
      for (iy = 1; iy < (ny+1); iy++) {
        ...
        used = fprint_spice_one_mux_testbench(formatted_spice_dir, circuit_name, mux_testbench_name, 
                                              include_dir_path, subckt_dir_path, LL_rr_node_indices,
                                              num_clocks, arch, ix, iy, CHANY, SPICE_CB_TB, 
                                              leakage_only);
        if (1 == used) {
          cnt += used;
        }
  ...
  case SPICE_SB_TB:
    ...
    for (ix = 0; ix < (nx+1); ix++) {
      for (iy = 0; iy < (ny+1); iy++) {
        ...
        used = fprint_spice_one_mux_testbench(formatted_spice_dir, circuit_name, mux_testbench_name, 
                                              include_dir_path, subckt_dir_path, LL_rr_node_indices,
                                              num_clocks, arch, ix, iy, NUM_RR_TYPES, SPICE_SB_TB, 
                                              leakage_only);
        if (1 == used) {
          cnt += used;
        }
