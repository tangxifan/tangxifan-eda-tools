...
$CONFIG_PATH = "${FPGA_FLOW_PATH}/configs/sample.conf";

sub print_usage() {
  print "\nThe configuration file is being generated. \nThe output is placed in ../configs/fpga_spice/sample.conf\n"; 
  return 1;
}
...
sub generate_file($) {
	...
	print $CONF_HANDLE "flow_type = standard #standard|mpack2|mpack1|vtr_standard|vtr|yosys_vpr\n";
