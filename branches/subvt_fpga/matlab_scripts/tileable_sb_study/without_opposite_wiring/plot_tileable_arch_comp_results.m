...
average_minW = mean(minW');
average_wirelength = mean(wirelength');
average_sb_mux_size = mean(sb_mux_size');
% normalize area, delay and power 
normalize_area = average_area./average_area(1);
normalize_delay = average_delay./average_delay(1);
normalize_minW = average_minW./average_minW(1);
normalize_wirelength = average_wirelength./average_wirelength(1);
normalize_sb_mux_size = average_sb_mux_size./average_sb_mux_size(1);

%% Bar graphs
% Normailized Graph
fig_handle0 = figure;
b = bar([normalize_area', normalize_delay', normalize_minW']); 
ch = get(b,'children');
