...
%% Data filling
sram_mux_2level = zeros( num_swept_wcpt, num_merits, num_swept_input_size, num_VDD); 
...
for isize = 1:1:num_swept_input_size
  ...
  for ivdd = 1:1:num_VDD 
    % find the best wcpt
    [best_delay, best_delay_index] = min(sram_mux_2level(:,2,isize,ivdd)); % the third column is delay
    [best_power, best_power_index] = min(sram_mux_2level(:,4,isize,ivdd)); % the fifth column is power
    [best_pdp, best_pdp_index]     = min(sram_mux_2level(:,5,isize,ivdd)); % the sixth column is PDP
    % For small input size, we fix the best delay index to be 1.
    % In these cases, the delay difference is small but the power difference is huge.
    if (isize < 4) 
      best_delay_index = 1;
    end
...
for isize = 1 : 1 : num_swept_input_size
  for ivdd = 1:1:num_VDD
    ...
    wcpt_opt_delay = sram_mux_2level_best_delay(1, isize, ivdd); % the second column is the Wprog
    area_multiplexing = ceil(sqrt(2 * isize)) * (trans_area(wcpt_opt_delay)+ trans_area(wcpt_opt_delay * pn_ratio));
    ...
    wcpt_opt_power = sram_mux_2level_best_power(1, isize, ivdd); % the second column is the Wprog
    area_multiplexing = ceil(sqrt(2 * isize)) * (trans_area(wcpt_opt_power)+ trans_area(wcpt_opt_power * pn_ratio));
    area_sram = one_sram_cell_area*(2*ceil(sqrt(2 * isize)));
    area_buf = (2 * isize + 1) * (trans_area(buf_trans_width)+ trans_area(buf_trans_width * pn_ratio));
    sram_mux_2level_best_power (1, isize, ivdd) = area_multiplexing + area_buf + area_sram;

    % For the case of best pdp
    wcpt_opt_pdp = sram_mux_2level_best_pdp(1, isize, ivdd); % the second column is the Wprog
    area_multiplexing = ceil(sqrt(2 * isize)) * (trans_area(wcpt_opt_pdp)+ trans_area(wcpt_opt_pdp * pn_ratio));
    area_sram = one_sram_cell_area*(2*ceil(sqrt(2 * isize)));
    area_buf = (2 * isize + 1) * (trans_area(buf_trans_width)+ trans_area(buf_trans_width * pn_ratio));
