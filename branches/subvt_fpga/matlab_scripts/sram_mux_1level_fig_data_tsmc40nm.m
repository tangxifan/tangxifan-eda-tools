...
%% Data filling
sram_mux_1level = zeros( num_swept_wcpt, num_merits, num_swept_input_size, num_VDD); 
...
for isize = 1:1:num_swept_input_size
  ...
  for ivdd = 1:1:num_VDD 
    % find the best wcpt
    [best_delay, best_delay_index] = min(sram_mux_1level(:,2,isize,ivdd)); % the third column is delay
    [best_power, best_power_index] = min(sram_mux_1level(:,4,isize,ivdd)); % the fifth column is power
    [best_pdp, best_pdp_index]     = min(sram_mux_1level(:,5,isize,ivdd)); % the sixth column is PDP
...
for isize = 1 : 1 : num_swept_input_size
  for ivdd = 1:1:num_VDD
    ...
    wcpt_opt_delay = sram_mux_1level_best_delay(1, isize, ivdd); % the second column is the Wprog
    ...
    wcpt_opt_power = sram_mux_1level_best_power(1, isize, ivdd); % the second column is the Wprog
    ...
    wcpt_opt_pdp = sram_mux_1level_best_pdp(1, isize, ivdd); % the second column is the Wprog
