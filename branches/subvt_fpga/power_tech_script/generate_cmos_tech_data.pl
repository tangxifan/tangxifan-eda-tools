...
sub get_nmos_leakage_for_Vds {
	...
	return &process_unit($results[0], "current");
}
...
sub get_mux_out_voltage {
    ...
    $s = $s . "R0a in outa 1k\n";
	$s = $s . "Xp0a in 0 0 0 nfet size='" . $size . "'\n";
	$s = $s . "Xpoa outa 0 0 0 nfet size='" . $size . "'\n";
	for ( my $i = 1 ; $i < $mux_size ; $i++ ) {
        $s = $s . "R" . $i . "a in outa 1000k\n";
		#$s = $s . "X" . $i . "a in 0 outa 0 nfet size='" . $size . "'\n";
	}
    $s = $s . "R0b in outb 1k\n";
	$s = $s . "Xp0b in 0 0 0 nfet size='" . $size . "'\n";
	$s = $s . "Xpob outb 0 0 0 nfet size='" . $size . "'\n";
	#$s = $s . "X0b in Vdd outb 0 nfet size='" . $size . "'\n";
	for ( my $i = 1 ; $i < $mux_size ; $i++ ) {
        $s = $s . "R" . $i . "b 0 outb 1000k\n";
